<template>
  <div>
    ...
    <div class="searchDiv">
      <modelForm v-model='postData[item.key]' :width='item.width' :size="item.size":key='item.key' :title='item.title' :items="getItems(item)" :type='item.type'
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      output() {
        tempetDownLoad('exportPolicys', this.postData,'已完成投保記錄報表')
      },
      givePower(obj){
      },
      queryList(index) {
