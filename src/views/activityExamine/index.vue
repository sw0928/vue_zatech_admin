<template>
 ...
 -->
		...
		<a-modal :width='1200' :title="isAdd ? '審核行銷活動' : '審核行銷活動明細'" :visible="isShowChange" @cancel="isShowChange = false">
				...
				<div class="content">
					...
					<a-form-model :label-col="labelCol" :wrapper-col="wrapperCol" v-if="beforeData != null">
						...
						<a-form-model-item label="跳轉連結">
							<a-input v-model="beforeData.activityUrl" disabled class="w_300" />
							<a-input v-model="afterData.activityUrl" :class="beforeData.activityUrl != afterData.activityUrl?'colorRed':''"
					...
					<a-form-model  :label-col="labelCol" :wrapper-col="wrapperCol" v-if="beforeData == null">
						...
						<a-form-model-item label="跳轉連結">
							<a-input v-model="afterData.activityUrl" disabled class="w_300 " />
						</a-form-model-item>
						<a-form-model-item label="通路名稱">
		...
		methods: {
			onSubmit() {
				...
				if(this.modifierShow){
					...
					this.Axios('approveActivity', tep)
						.then((res) => {
							//console.log(res)
			...
			toDetail(obj) {
				...
				this.isAdd = false;
				this.Axios('getApproveActivityDetail', obj)
					.then((res) => {
			...
			queryList(index) {
				...
				this.Axios('activityApproveListPage', tep)
					.then((res) => {
