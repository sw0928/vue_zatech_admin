<template>
  ...
  <div class="tab">
    ...
    <tableColumn v-if="is_chose==1" :columns="columns" :columnsData="columnsData" :pageInfo="pageInfo" :totalNum="totalNum" :openExpand="false">      
        ...
        <span class="iconfont icon-iconset0414" :class="props.record.sortFlag == 1?'noColor':''" @click="upperShelf(props.record.activityId,1,props.index)"></span>
        <span class="iconfont icon-iconset0413" :class="props.record.sortFlag == 3?'noColor':''" @click="upperShelf(props.record.activityId,2,props.index)"></span>
      </span>
			<span slot="operation" slot-scope="props">
        <a-button @click="edit(props.record.activityId,props.record)">修改</a-button>
...
//       },
    ...
    methods: {
      ...
      queryList(index) {
				...
				if(tep.channelCode == undefined || tep.channelCode == ""){
					this.sortShow = false
				}else{
					this.sortShow = true
				}
				...
				if(this.is_chose == 1){
					this.Axios('serviceListPage',tep)
					.then((res) => {
					    this.columnsData = res.data.data.list;							
					    this.totalNum = res.data.data.total;						
					})
				}else{
					this.Axios('serviceModifListPage',tep)
					.then((res) => {
					    this.columnsData = res.data.data.list;
					    this.totalNum = res.data.data.total
					})
