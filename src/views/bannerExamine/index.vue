<template>
	<div class="">
		...
		<div class="searchDiv">
			...
			<modelForm v-model='postData[item.key]' :size="item.size" :width='item.width' :key='item.key' :title='item.title'
			 :items="getItems(item)" :type='item.type' v-for="item in searchArr" class='comDiv' :selectHour=item.selectHour
			 :isAutoPlacehoder='false'></modelForm>
			<div class="searchBTn">
 ...
 -->
		...
		<a-modal :width='1200' :title="isAdd ? '審核廣告資料' : '審核廣告資料明細'" :visible="isShowChange" @cancel="isShowChange = false">
			...
			<div class="examine">
				<div class="content">
					...
					<a-form-model :label-col="labelCol" :wrapper-col="wrapperCol" v-if="beforeData != null">
						...
						<a-form-model-item label="跳轉連結">
							<a-input v-model="beforeData.bannerLink" disabled class="w_300" />
							<a-input v-model="afterData.bannerLink" :class="beforeData.bannerLink != afterData.bannerLink?'colorRed':''"
							 disabled class="w_300 marL_200" />
						</a-form-model-item>
						<a-form-model-item label="通路名稱">
							<a-input v-model="beforeData.channelName" disabled class="w_300" />
							<a-input v-model="afterData.channelName" :class="beforeData.channelName != afterData.channelName?'colorRed':''"
							 disabled class="w_300 marL_200" />
						</a-form-model-item>
						<a-form-model-item label="通路代碼">
							<a-input v-model="beforeData.channelCode" disabled class="w_300" />
							<a-input v-model="afterData.channelCode" :class="beforeData.channelCode != afterData.channelCode?'colorRed':''"
							 disabled class="w_300 marL_200" />
						</a-form-model-item>
						...
						<a-form-model-item label="發佈時間">
							<div class="sw_date ">
								<a-input v-model="beforeData.grantBegStr" disabled style="width: 40%;padding: 4px 5px;" />--
								<a-input v-model="beforeData.grantEndStr" disabled style="width: 40%;padding: 4px 5px;" />
							</div>
							<div class="sw_date" style="margin-left: 13%;">

								<a-input v-model="afterData.grantBegStr" :class="beforeData.grantBegStr != afterData.grantBegStr?'colorRed':''"
								 disabled style="width: 40%;padding: 4px 5px;" />--
						...
						<a-form-model-item label="審核">
							<a-radio-group v-model="ruleForm.auditorStatus" v-if="afterData.auditorStatus == '1'" :disabled="modifierShow"
							 class="w_300">
								<a-radio value="3">
									通過
								</a-radio>
					...
					<a-form-model :label-col="labelCol" :wrapper-col="wrapperCol" v-if="beforeData == null">
						<a-form-model-item label="所屬欄位">
							<a-select v-model="afterData.position" disabled class="w_300 ">
								<a-select-option :value="itme.code" v-for="(itme, index) in positionL" :key="index">{{itme.value}}</a-select-option>
							</a-select>
						</a-form-model-item>
						<a-form-model-item label="標題">
						...
						<a-form-model-item label="跳轉連結">
							<a-input v-model="afterData.bannerLink" disabled class="w_300 " />
						</a-form-model-item>
						<a-form-model-item label="通路名稱">
							<a-input v-model="afterData.channelName" disabled class="w_300 " />
						</a-form-model-item>
						<a-form-model-item label="通路代碼">
							<a-input v-model="afterData.channelCode" disabled class="w_300 " />
						</a-form-model-item>
						<a-form-model-item label="發佈狀態">
						...
						<a-form-model-item label="發佈時間">
							<div class="sw_date">
								<a-input v-model="afterData.grantBegStr" disabled style="width: 40%;padding: 4px 5px;" />--
								<a-input v-model="afterData.grantEndStr" disabled style="width: 40%;padding: 4px 5px;" />
							</div>
						</a-form-model-item>							
						<a-form-model-item label="審核">
	...
	export default {
		...
		methods: {
			...
			toDetail(obj) {
				...
				this.isAdd = false;
				this.Axios('getApproveDetail', obj)
					.then((res) => {
			...
			queryList(index) {
				...
				this.Axios('approveListPage', tep)
					.then((res) => {
